...
struct variable_list_st *variable_list_list_last(struct variable_list_list_st *res) {
    if (res == NULL || res->variable_lists == NULL || res->size == 0) return NULL;
    return res->variable_lists[res->size - 1];
}

// TLV Methods
int8_t variable_list_list_set_tlv(struct variable_list_list_st *res, const struct string_st *tlv) {
    if (res == NULL) return ERR_DATA_NULL;
    variable_list_list_clear(res);
    int32_t tag = tlv_get_tag(tlv);
    if (tag < 0) return (int8_t) tag;
    if (tag != TLV_VARIABLE_LIST_LIST) return ERR_TLV_TAG;
