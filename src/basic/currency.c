...
int8_t currency_cmp(const struct currency_st *obj1, const struct currency_st *obj2) {
...
// TLV Methods
int8_t currency_set_tlv(struct currency_st *res, const struct string_st *tlv) {
    if (res == NULL) return ERR_DATA_NULL;
    currency_clear(res);
    int32_t tag = tlv_get_tag(tlv);
    if (tag < 0) return (int8_t) tag;
    if (tag != TLV_CURRENCY) return ERR_TLV_TAG;
...
void currency_get_tlv(const struct currency_st *currency, struct string_st *res) {
    ...
    size_get_tlv(currency->hash_type, &_tlv_data);
    string_concat(res, &_tlv_data);

    size_get_tlv(currency->crypto_type, &_tlv_data);
    string_concat(res, &_tlv_data);

    size_get_tlv(currency->crypto_base, &_tlv_data);
    string_concat(res, &_tlv_data);
...
(struct error_st *err, const struct currency_st *currency, const struct string_st *str) {
    ...
    if (str->size == 4 && memcmp(str->data, "name", 4) == 0) {
        ...
        integer_set_ui(res->data, currency->hash_type);
    }
    else if (str->size == 11 && memcmp(str->data, "crypto_type", 11) == 0) {
        object_set_type(res, INTEGER_TYPE);
        integer_set_ui(res->data, currency->crypto_type);
    }
    else if (str->size == 11 && memcmp(str->data, "crypto_base", 11) == 0) {
        object_set_type(res, INTEGER_TYPE);
        integer_set_ui(res->data, currency->crypto_base);
    }
